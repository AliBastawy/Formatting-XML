#include "formatting.h"
#include "ui_formatting.h"
#include "mainwindow.h"
#include <QScreen>
#include <QDebug>
#include <QFileDialog>

#include <iostream>
#include <math.h>
#include <cmath>
#include <string>
#include <stdlib.h>
#include <stdio.h>
#include "Nodes_Tree.h"
#include <fstream>
#include "tree.h"
#include "XML_Formater.h"


#include "options.h"
#include "consistency.h"
#include "tojson.h"
#include "testdialog.h"

using namespace std;

Options *fOptions;
Consistency *fConsistency;
ToJSON *fToJSON;
TestDialog *testDialogF;

vector<string> Formatting::formattedXML;

/*
//vector<string> Xml_FileCorrection2(vector<string>& Xml_File)
//{

//    int lines_iterator=0;
//    int lines_Number=Xml_File.size();
//    vector<string> errors_contained;
//    string line;
//    stack<tags> collector;

//        //the file iterator
//        while (lines_iterator < lines_Number)
//        {
//            line = Xml_File[lines_iterator];
//            int character_iterator=0;
//            //the line iterator
//            while(line[character_iterator]!='\0')
//            {
//                if(line[character_iterator]=='<')
//                {
//                    string tag;
//                    if(line[character_iterator+1]=='/')
//                    {
//                        character_iterator=character_iterator+2;
//                        while(line[character_iterator-1]!='>')
//                        {
//                            tag.push_back(line[character_iterator++]);
//                        }
//            CheckOnTag:
//                        if(!collector.empty())
//                        {
//                            if(tag == (collector.top()).data)
//                            {
//                                collector.pop();
//                            }
//                            else
//                            {
//                                bool missing_openingtag=1;
//                                stack<tags> temp_collector =collector;
//                                temp_collector.pop();
//                                while(!(temp_collector.empty()))
//                                {
//                                    //check if the missing is a closing tag
//                                    if(tag==((temp_collector.top()).data))
//                                    {
//                                        //the correction of the missing closing tag
//                                        tags temp_tag = collector.top();
//                                        collector.pop();
//                                        string temp_line=Xml_File[temp_tag.line];
//                                        temp_tag.data = "</" + temp_tag.data;
//                                        temp_line.insert(temp_tag.end,temp_tag.data);
//                                        Xml_File[temp_tag.line]=temp_line;



//                                        //sends the error to the GUI window
//                                        errors_contained.push_back("# There Is A Missing Closing Tag "+temp_tag.data);
//                                        missing_openingtag=0;
//                                        goto CheckOnTag;
//                                    }
//                                    temp_collector.pop();
//                                }
//                                if (missing_openingtag==1)
//                                {
//                                    //the correction of the missing opening tag
//                                    tags temp_tag = collector.top();
//                                    string temp_line=Xml_File[temp_tag.line];
//                                    tag= "<" + tag;
//                                    temp_line.insert(temp_tag.end,tag);
//                                    Xml_File[temp_tag.line]=temp_line;

//                                    //sends the error to the GUI window
//                                    errors_contained.push_back("# There Is A Missing Opening Tag "+tag);
//                                }
//                            }
//                        }
//                        else
//                        {
//                            //the correction of the missing opening tag while the stack is empty
//                            string temp_line=Xml_File[0];
//                            tag = "<" + tag;
//                            temp_line.insert(0,tag);
//                            Xml_File[0]=temp_line;

//                            //sends the error to the GUI window
//                            errors_contained.push_back("# There Is A Missing Opening Tag "+tag);
//                        }
//                    }
//                    else
//                    {
//                       if(line[character_iterator+1]!='?')
//                       {
//                            //get the opening tag and push it in the stack with all the required data about it
//                            tags *ptr;
//                            character_iterator=character_iterator+1;
//                            while(line[character_iterator-1]!='>')
//                            {
//                                if(line[character_iterator]==' ')
//                                {
//                                    tag.push_back('>');
//                                    break;
//                                }
//                                tag.push_back(line[character_iterator++]);
//                            }

//                                ptr = new tags(tag,lines_iterator,character_iterator);
//                                collector.push(*ptr);
//                       }
//                       else
//                       {
//                           break;
//                       }

//                    }
//                }
//                else
//                {
//                    character_iterator++;
//                }
//            }
//            lines_iterator++;
//        }
//        // handling the missing closing tags of the tags in the stack
//        while(!(collector.empty()))
//        {
//            //the correction of the missing closing tag
//            string temp_tag = (collector.top()).data;
//            collector.pop();
//            temp_tag = "</" + temp_tag;
//            Xml_File.push_back(temp_tag);

//            //sends the error to the GUI window
//            errors_contained.push_back("# There Is A Missing Closing Tag "+temp_tag);

//        }

//        // return a vector containing all the correction done
//        return errors_contained;
//}


//vector<string> Xml_FileOpener2(string file_path)
//{

//    string inline1;
//    vector<string> openedxmlfile;
//    ifstream xmlfile;
//    xmlfile.open(file_path);

//    if (!xmlfile.fail())
//    {
//        while(!xmlfile.eof())
//        {
//            getline(xmlfile,inline1);
//            openedxmlfile.push_back(inline1);
//        }

//    }
//    xmlfile.close();
//    return openedxmlfile;
//}
*/


Formatting::Formatting(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::Formatting)
{
    ui->setupUi(this);
    ui->textEdit->setPlainText(MainWindow::getXML());
    ui->textEdit->setReadOnly(true);
    ui->textEdit_2->setReadOnly(true);
//    move(screen()->geometry().center() - frameGeometry().center());
    move(pos() + (QGuiApplication::primaryScreen()->geometry().center() - geometry().center()));


    QPixmap pic(":/Images/next.png");
    ui->label_3->setPixmap(pic);
    ui->label_3->setScaledContents(true);


    // Function of Formatting

    // Check First if there is an error in the XML file or not

//    // Start Consistency Function

//    string Filepath2 = MainWindow::getFilePath();
//    vector <string> xmlfile2;
//    xmlfile2 = Xml_FileOpener2(Filepath2);

//    vector<string> corrections2 = Xml_FileCorrection2(xmlfile2);

//    // Put Correction String in Error Detected Text Edit
//    string correction2;

//    if (corrections2.size() != 0)
//    {

//        for(int i =0; i < corrections2.size(); i++)
//        {
//            correction2 += corrections2.at(i) + "\n";
//        }
//    }



    ifstream inputFile(MainWindow::getFilePath());
        XML_Formater f1;

//    ofstream outputfile("output.xml");

    vector<string> temp;

    f1.xml_cutter(inputFile, &temp);

//    ifstream inputfie_after_cutting("output.xml");

//    ofstream File4("XMLFile.xml");


    class tree t1;
    Nodes_Tree * t2;
    t2 = new Nodes_Tree();


    t1.tree_creator(t2, &temp, &Formatting::formattedXML);

    string formattedXMLString;

    for (int i = 0; i < Formatting::formattedXML.size(); i++)
    {
        formattedXMLString += Formatting::formattedXML.at(i) + "\n";
    }
    ui->textEdit_2->setPlainText(QString::fromStdString(formattedXMLString));

}

Formatting::~Formatting()
{
    delete ui;
}

void Formatting::on_actionOpen_triggered()
{
    QString file = QFileDialog::getOpenFileName(this, "Open a File");
    MainWindow::setFilePath(file);

    if (!file.isEmpty() && file.toStdString().substr(file.length()-3) == "xml")
    {
        QFile sFile(file);
        if (sFile.open(QFile::ReadOnly | QFile::Text))
        {
            MainWindow::openFunction(&sFile);

            sFile.close();

            ui->textEdit->setPlainText(MainWindow::getXML());

            // Function of Formatting

            ifstream inputFile(MainWindow::getFilePath());
                XML_Formater f1;

        //    ofstream outputfile("output.xml");

            vector<string> temp;

            f1.xml_cutter(inputFile, &temp);

        //    ifstream inputfie_after_cutting("output.xml");

        //    ofstream File4("XMLFile.xml");


            class tree t1;
            Nodes_Tree * t2;
            t2 = new Nodes_Tree();

            Formatting::formattedXML.clear();

            t1.tree_creator(t2, &temp, &Formatting::formattedXML);

            string formattedXMLString;

            qDebug() << "TEST";

            for (int i = 0; i < Formatting::formattedXML.size(); i++)
            {
                formattedXMLString += Formatting::formattedXML.at(i) + "\n";
            }
            ui->textEdit_2->setPlainText(QString::fromStdString(formattedXMLString));

        }
    }
}


void Formatting::on_pushButton_clicked()
{
    hide();
    fOptions = new Options(this);
    fOptions->show();
}

void Formatting::on_actionConsistency_triggered()
{
    hide();
    fConsistency = new Consistency(this);
    fConsistency->show();
}


void Formatting::on_actionXML_To_JSON_triggered()
{
    hide();
    fToJSON = new ToJSON(this);
    fToJSON->show();
}


void Formatting::on_actionCompress_triggered()
{
    testDialogF = new TestDialog(this);
    testDialogF->setModal(true);
    testDialogF->show();
}

void Formatting::on_actionSave_triggered()
{
    QString s = QFileDialog::getSaveFileName(
    this,
    "save formatted file",
    "output.xml");
}

